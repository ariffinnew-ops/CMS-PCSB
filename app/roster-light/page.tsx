...
export default function RosterLightPage() {
  ...
  return (
    <AppShell>
      ...
      {/* Light mode wrapper */}
      <div className="space-y-4 animate-in fade-in duration-500 mt-4 bg-gray-50 -mx-4 -my-4 p-4 min-h-screen">
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2 bg-white p-2 rounded-xl shadow-sm border border-gray-200">
            <select
              ...
              className="bg-gray-100 border-none rounded-lg px-3 py-1.5 text-[10px] font-bold text-slate-700 uppercase outline-none"
            >
                  ...
                  {daysInMonth.map((d) => (
                    <th
                      ...
                      className={`px-0 py-1.5 text-center w-[26px] min-w-[26px] max-w-[26px] border-r border-gray-200 ${
                        d.isWeekend ? "bg-gray-150" : "bg-gray-100"
                      }`}
                      style={{ backgroundColor: d.isWeekend ? '#f1f5f9' : '#f9fafb' }}
                    >
                ...
                {loading ? (
                  ...
                  groupedData.map((item, idx) => {
                    if (item.type === 'separator') {
                      // Light separator - subtle grey with dark text
                      return (
                        <tr key={`sep-${idx}`} className="bg-gray-100">
                          <td 
                            colSpan={daysInMonth.length + 1} 
                            className="px-3 sticky left-0 z-10 border-t border-b border-gray-200"
                            style={{ height: '22px' }}
                          >
                    ...
                    return (
                      ...
                      >
                        ...
                        {daysInMonth.map((d) => {
                          ...
                          // Light flat bar styles - no shadows, clean colors
                          let barClass = "";
                          if (status === "PRIMARY" || status === "OHN_WEEKDAY") {
                            // Medium Blue for Primary / OHN Weekday
                            barClass = "bg-blue-500";
                          } else if (status === "SECONDARY") {
                            // Soft Sky Blue for EM Secondary
                            barClass = "bg-sky-300";
                          } else if (status === "OHN_WEEKEND") {
                            // Light Slate/Greyish Blue for OHN Weekend
                          ...
                          return (
                            <td
                              ...
                              style={{ 
                                height: '28px',
                                backgroundColor: d.isWeekend ? '#f8fafc' : '#ffffff'
                              }}
                            >
