...
export default function AdminPage() {
  ...
  return (
    <AppShell>
      <div className="flex flex-col h-full max-h-[calc(100vh-80px)] mt-0.5 relative gap-0">
        {/* SYNCING INDICATOR */}
        {isSyncing && (
        ...
        {/* TIMELINE NAVIGATION */}
        <div className="bg-blue-600 backdrop-blur-md px-3 py-1 border-b border-blue-700 flex items-center gap-2 flex-shrink-0 shadow-md rounded mx-1 mt-0.5">
        ...
        {/* LAST SYNCED STATUS */}
        {lastSynced && (
          <div className="flex items-center justify-end px-3 py-0.5 bg-muted/30 flex-shrink-0">
        ...
        <div className="bg-card overflow-hidden flex flex-col flex-[1] max-h-[50vh]">
          <div
        ...
        {sortedData.length > 0 && (() => {
          ...
          return (
            <div className="overflow-hidden flex flex-col flex-[5] min-h-0">
              ...
              `}</style>
              <div className="overflow-auto flex-1 min-h-0 bg-card">
                <table className="w-full border-collapse" style={{ minWidth: `${192 + daysCount * 26}px` }}>
                  <colgroup>
                    <col style={{ width: "192px", minWidth: "192px" }} />
                    {dayHeaders.map(d => <col key={d.dayNum} style={{ width: "26px", minWidth: "26px", maxWidth: "26px" }} />)}
                  </colgroup>
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-slate-100">
                      <th className="px-2 py-1 text-left sticky left-0 z-20 border-r border-gray-300" style={{ backgroundColor: "#1e3a8a" }}>
                        <div className="flex items-center justify-between gap-1.5">
                          <div className="flex items-center gap-1.5 min-w-0">
                            <svg className="w-3.5 h-3.5 text-white shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                            <span className="text-[9px] font-black text-white uppercase tracking-wider whitespace-nowrap">Movement Register</span>
                            <span className="text-[8px] font-bold text-blue-200 whitespace-nowrap">{sortedData.length}</span>
                          </div>
                          <select
                            value={extractMonth}
                      ...
                      {dayHeaders.map(d => (
                        <th key={d.dayNum} className={`px-0 py-0.5 text-center border-r border-gray-200 ${d.isWeekend ? "bg-gray-100" : "bg-slate-50"}`}>
                          <div className="flex flex-col items-center leading-none">
                            <span className="text-[8px] font-bold text-slate-500">{d.dayName}</span>
                            <span className="text-[10px] font-black text-slate-700 tabular-nums">{d.dayNum}</span>
                          </div>
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {grouped.map((item, idx) => {
                      ...
                      const row = item.row!;
                      return (
                        <tr key={`grow-${row.crew_id}-${row.crew_name}`} className="hover:bg-blue-50/50 transition-colors group border-b border-gray-200" style={{ height: "22px" }}>
                          <td className="px-2 py-0 sticky left-0 bg-slate-100 group-hover:bg-blue-100/50 z-10 border-r border-gray-300">
                            <div className="text-[8px] font-semibold text-slate-700 leading-tight truncate" title={getDisplayName(row)}>{getDisplayName(row)}</div>
                          </td>
                          {dayHeaders.map(d => {
